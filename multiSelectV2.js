 ...
 *  ║  • Light/hover/selected backgrounds                       ║
 *  ║  • Configurable chat enable/disable with instant tooltip  ║
...
export const MultiSelect = {
  ...
  // Trigger only on traces of type 'multi_select'
  match: ({ trace }) => trace.payload && trace.type === 'multi_select',

  render: ({ trace, element }) => {
    try {
      /* ══ 0. Utility functions (must be declared first) ═════════════════ */
      ...
      const {
        ...
        chatDisabledText = '🚫'         // tooltip when chat disabled
      } = trace.payload;

      /* ══ 2. Create container early so patchInteract can close it ════════ */
      const container = document.createElement('div');
      container.classList.add('multiselect-container');
      if (sections.length === 1) container.classList.add('one-section');

      /* ══ 3. Helpers to disable/enable built-in chat input ═══════════════ */
      const disableChatInput = () => {
        const root = element.getRootNode();
        const ic = root.querySelector('.vfrc-input-container');
        if (!ic) return;
        // grey out and show not-allowed cursor
        ic.style.opacity = '.5';
      ...
      /* ══ 5. Patch voiceflow.chat.interact to gray-out widget upon user text ═════ */
      if (chat && window.voiceflow?.chat?.interact) {
        ...
        window.voiceflow.chat.interact = args => {
          if (args.type === 'text' && args.payload?.text) {
            // gray out the multiselect container
...
.multiselect-container.disabled-container {
  ...
  pointer-events: none !important;
      ...
      sections.forEach(sec => {
        ...
        // apply section background + a lighter accent
      ...
      /* ══ 11. Finally insert into the page ═══════════════════════════ */
