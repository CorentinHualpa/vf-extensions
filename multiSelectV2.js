...
export const MultiSelect = {
  ...
  render: ({ trace, element }) => {
    try {
      /* ══ 1. Lecture du payload, y compris chat + chatDisabledText ═════════ */
      const {
        sections         = [],
        buttons          = [],
        totalMaxSelect   = 0,
        multiselect      = true,
        chat             = true,                      // autoriser/disallow chat
        chatDisabledText = '🚫'                       // texte hover quand chat=false
      } = trace.payload;
      ...
      // Désactive la zone de chat intégrée
      ...
      // Réactive la zone de chat intégrée
      ...
      /* ══ 3. Désactive le chat si demandé ═══════════════════════════════════ */
      if (!chat) disableChatInput();

      /* ══ 4. Patch chat.interact pour désactiver widget si l'user écrit ═══ */
      ...
      /* ══ 5. Création du container de l’extension ═════════════════════════════ */
      const container = document.createElement('div');
      container.classList.add('multiselect-container');
      if (sections.length === 1) container.classList.add('one-section');

      /* ───────────────────────────────────────────────────────────────────── */
      /*  CSS interne (variables, layout, styles)                            */
      /* ───────────────────────────────────────────────────────────────────── */
...
.multiselect-container.disabled-container{
  opacity:.5!important; pointer-events:none!important;
      ...
      /* ══ 7. Création récursive des options ═══════════════════════════════════ */
      const createOptionElement = opt => {
        ...
        inp.addEventListener('change', () => {
          ...
          [...rootOL.querySelectorAll('.option-container label')].forEach(l =>
            l.classList.toggle('selected', l.querySelector('input').checked)
          );
          // single-select → envoi direct
          if (!multiselect) {
            enableChatInput();
            container.classList.add('disabled-container');
      ...
      /* ══ 8. Construction de chaque section ═════════════════════════════════ */
      ...
      sections.forEach(sec => {
        ...
        // couleur de fond + accent dynamique
