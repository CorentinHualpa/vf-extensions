  ...
  render: ({ trace, element }) => {
    try {
      ...
      const disableChatInput = () => {
        ...
        if (!ic) return;
        // style to grey it out & block clicks
        ic.style.pointerEvents = 'none';
        ic.style.opacity       = '.5';
        ic.style.cursor        = 'not-allowed';
        // add tooltip text
        ic.setAttribute('title', chatDisabledText);
        // also apply to textarea & send button
      ...
      // Initially disable if chat=false
      if (!chat) disableChatInput();

      // Patch chat.interact to grey-out widget if user bypasses
      ...
      /* ══ 3. Create root container for the widget ═════════════════════════════ */
      const container = document.createElement('div');
      container.classList.add('multiselect-container');
      if (sections.length === 1) container.classList.add('one-section');

      /* ══ 4. Inject widget-specific CSS and variables ═════════════════════════ */
...
.multiselect-container .option-container input[type="radio"] {
  ...
  flex-shrink: 0 !important;        /* keep circle size */
...
.multiselect-container.disabled-container {
  ...
  pointer-events: none !important;
      ...
      /* ══ 6. Factory to create each option ═════════════════════════════════ */
