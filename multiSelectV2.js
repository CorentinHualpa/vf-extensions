 ...
 *  ║  • chaque menu reçoit un `menuIndex` unique              ║
 *  ╚═══════════════════════════════════════════════════════════╝
 */
export const MultiSelect = {
  ...
  render: ({ trace, element }) => {
    try {
      ...
      /* 1. générer un index unique pour CE menu */
      const menuIndex = `${Date.now()}_${Math.random().toString(36).substr(2,9)}`;

      /* 2. désactiver tout ancien menu */
      ...
      /* 6. griser à l'envoi d'un message (Enter ou bouton) */
      if (chat) {
...
.multiselect-container.disabled-container { opacity:.5!important; pointer-events:none!important; }
      ...
      /* 9. création récursive d’une option */
      const createOptionElement = opt => {
        ...
        inp.addEventListener('change', () => {
          ...
          // griser/réactiver si action==="all"
          if (opt.action === 'all') {
      ...
      /* 12. insertion dans le DOM */
      element.appendChild(container);
      console.log('✅ MultiSelect prêt (index:', menuIndex, ')');
    } catch (err) {
